...
{$DEFINE KOL_MCK} // Use only in none-MCK projects (in ones it add's automaticaly)
{.$UNDEF KOL_MCK} // Use only in MCK projects if you want none-KOL version 
...
{$ENDIF}
 ...
 TRegExpr = class
   ...
   {$IFDEF KOL_MCK}
   procedure Split (AInputStr : RegExprString; APieces : PStrListEx);
{$ELSE}
...
{$IFDEF KOL_MCK}
   procedure SplitRegExpr (const ARegExpr, AInputStr : RegExprString; APieces : PStrListEx);
{$ELSE}
...
{$IFDEF KOL_MCK}
 ...
 ASubExprs : PStrListEx; AExtendedSyntax : boolean{$IFDEF D4_}= False{$ENDIF}) : integer;
{$ELSE}
   function RegExprSubExpressions (const ARegExpr : string;
...
//  SetLength (Stack, StackSz); //###0.945
  ...
  while (i <= Len) do begin
   ...
   ASubExprs.Objects [SubExprIdx] :=
              DWORD(TObject (StartPos or (SubExprLen ShL 16)));
{$ELSE}
   ASubExprs.Objects [SubExprIdx] :=
...
{$IFDEF KOL_MCK}
   ASubExprs.Items [SubExprIdx] := System.Copy (
...
{$ENDIF}
  ...
  if (ASubExprs.Count = 0)
   ...
   or (((integer (ASubExprs.Objects [0]) ShR 16) and $FFFF) <> Len)
    ...
    then
    {$IFDEF KOL_MCK}
   ASubExprs.InsertObject (0, ARegExpr, DWORD(TObject ((Len ShL 16) or 1)));
{$ELSE}
...
--------------------------------------------------------------}
    ...
    {$IFDEF KOL_MCK}
procedure TRegExpr.Split (AInputStr : RegExprString; APieces : PStrListEx);
{$ELSE}
       ...
       for Ch := #0 to #255 do
        if Ch in PSetOfREChar (s)^ then
         if Ch < ' '
          then Result := Result + '#' +
                     {$IFDEF KOL_MCK}
Int2Str (Ord (Ch)) //###0.936
{$ELSE}
IntToStr (Ord (Ch)) //###0.936
{$ENDIF}
